...
app.get("/", (req, res) => {
  res.json({
    ...
    endpoints: {
      ...
      "GET /api/vote-dashboard": "Get live vote dashboard",
...
app.post("/api/votes", async (req, res) => {
  try {
    ...
    const requiredFields = ['name', 'state', 'district', 'assembly', 'voterId', 'candidate', 'party', 'voteId'];
    ...
    const vote = new VoteCast(data);
    const savedVote = await vote.save();

    // Update or create dashboard entry for the party
    await Dashboard.findOneAndUpdate(
      { partyName: data.party },
      { $inc: { voteCount: 1 }, lastUpdated: Date.now() },
    ...
    if (err.code === 11000) {
      return res.status(400).json({ 
        success: false,
        error: "Vote already cast for this voterId" 
      });
    }
    res.status(500).json({ 
      success: false,
...
// Get live vote dashboard
app.get("/api/vote-dashboard", async (req, res) => {
  try {
    // Aggregate votes from VoteCast collection
    const voteSummary = await VoteCast.aggregate([
      {
        $group: {
          _id: "$party", // Group by party
          voteCount: { $sum: 1 } // Count votes for each party
        }
      }
    ]);

    // Optionally update Dashboard collection with aggregated data
    for (const summary of voteSummary) {
      await Dashboard.findOneAndUpdate(
        { partyName: summary._id },
        { voteCount: summary.voteCount, lastUpdated: Date.now() },
        { upsert: true, new: true }
      );
    }

    // Return the latest data (either from aggregation or Dashboard)
    const dashboardData = await Dashboard.find().lean();
    res.status(200).json({
      success: true,
      data: dashboardData
    });
  } catch (err) {
    console.error("Get vote dashboard error:", err);
    res.status(500).json({ success: false, error: "Failed to fetch vote dashboard" });
  }
});
...
app.use(/.*/, (req, res) => {
  res.status(404).json({
    ...
    availableSystems: ["voters", "applications", "votes"]
  });
});
...
app.listen(PORT, () => {
  ...
  console.log(`🗳️ Vote Casting System: Active`);
